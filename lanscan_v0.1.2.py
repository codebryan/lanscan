#!/usr/bin/env python3
"""
LAN Proxy Scanner v0.1.2 - 區域網路住宅代理節點偵測工具
...
# ─── 埠口掃描 ─────────────────────────────────────────────────────────────────
#
# v0.1.2 使用 blocking settimeout() + connect()
# CPython 在 socket.connect() 內部自行處理 EINPROGRESS，
# 比手動 non-blocking + select 更可靠，且跨平台一致。
# ─────────────────────────────────────────────────────────────────────────────

def scan_port(ip, port, timeout=1.5):
    """blocking settimeout + connect（CPython 內部處理 EINPROGRESS）"""
    s = None
    try:
        ...
        dbg(f":{port} unexpected exception: {type(e).__name__}: {e}")
        return False, ""
    finally:
        if s:
            try:
...
def test_single_port(host, port, timeout=3.0):
    ...
    # ── 方法 2: non-blocking connect_ex() + select() ───────────────────────
    ...
    try:
        ...
        if err == 0:
            ...
            t0 = time.monotonic()
            try:
                r_set, w_set, _ = select.select([], [s], [], timeout)
                elapsed = time.monotonic() - t0
                print(f"  步驟   │ select() 回傳: readable={bool(r_set)} writable={bool(w_set)}  耗時 {elapsed:.3f}s")
                if w_set:
                    so_err = s.getsockopt(socket.SOL_SOCKET, socket.SO_ERROR)
                    print(f"  步驟   │ SO_ERROR = {so_err} ({_errno_name(so_err) if so_err else 'OK'})")
                    if so_err == 0:
                        print(f"  結果   │ \033[92mOPEN\033[0m")
                    else:
                        print(f"  結果   │ CLOSED  (SO_ERROR={so_err} {_errno_name(so_err)})")
                else:
                    print(f"  結果   │ CLOSED/FILTERED  (select timeout)")
            except Exception as se:
                elapsed = time.monotonic() - t0
                print(f"  步驟   │ select() 拋出例外 {elapsed:.3f}s 後: {type(se).__name__}: {se}")
                print(f"  結果   │ ERROR")
        else:
            print(f"  結果   │ CLOSED  (connect_ex 立即返回非 EINPROGRESS)")
    except Exception as e:
        print(f"  結果   │ EXCEPTION  {type(e).__name__}: {e}")
    finally:
        if s:
    ...
    try:
        ...
        result_str = "\033[92mOPEN\033[0m" if rc == 0 else "CLOSED"
        print(f"  結果   │ {result_str}  (returncode={rc})")
    ...
    if port in (80, 443, 8080, 8443, 8888):
        ...
        print(f"\n  方法 4 │ curl {url} (port {port} 是 HTTP 類型)")
        try:
            r = subprocess.run(
...
範例:
  ...
  sudo python3 lanscan_v{VERSION}.py --fast                # 只掃代理/VPN 埠
  sudo python3 lanscan_v{VERSION}.py --debug               # 網路偵測除錯
       python3 lanscan_v{VERSION}.py --test-port 10.0.0.1 80   # 單埠三方法診斷
